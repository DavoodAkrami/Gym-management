          ...
          {programs.map((program) => (
            <li key={program.id} className="panel-card p-4">
              ...
              <button
                ...
                className="btn-primary mt-3 rounded-xl px-4 py-2 text-sm font-black disabled:opacity-70"
                onClick={() => void handleJoin(program.id)}
              >
                {joiningId === program.id ? <Spinner label={t("uiSaving")} /> : t("memberProgramJoin")}
              </button>
            </li>
          ))}
        </ul>
